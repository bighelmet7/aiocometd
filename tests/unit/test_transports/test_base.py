...
class TestTransportBase(TestCase):
    def setUp(self):
        self.transport = TransportBaseImpl(
            url="example.com/cometd", incoming_queue=None, http_session=None, loop=None
        )
    ...
    def test_init_with_loop(self):
        transport = TransportBaseImpl(
            url=None, incoming_queue=None, http_session=None, loop=None
        )
    ...
    def test_init_without_loop(self, asyncio_mock):
        transport = TransportBaseImpl(url=None, incoming_queue=None, http_session=None)
    ...
    def test_init_with_reconnect_advice(self):
        ...
        transport = TransportBaseImpl(
            url=None, incoming_queue=None, http_session=None, reconnect_advice=advice
        )
    ...
    def test_update_subscriptions_new_subscription_success(self):
        ...
        self.assertEqual(
            self.transport.subscriptions, set([response_message["subscription"]])
        )
    ...
    async def test_consume_payload_matching_without_advice(self, is_matching_response):
    ...
    async def test_consume_payload_matching_without_advice_extension(
        ...
        is_matching_response.assert_called_with(payload[0], message)
        self.transport._consume_message.assert_not_called()
        self.transport._process_incoming_payload.assert_called_with(payload, headers)

    @mock.patch("aiocometd.transports.base.is_matching_response")
    async def test_consume_payload_matching_with_advice(self, is_matching_response):
        payload = [
            {
        ...
        self.assertEqual(result, payload[0])
        self.assertEqual(self.transport.reconnect_advice, payload[0]["advice"])
        self.transport._update_subscriptions.assert_called_with(payload[0])
        is_matching_response.assert_called_with(payload[0], message)
        self.transport._consume_message.assert_not_called()
        self.transport._process_incoming_payload.assert_called_with(payload, None)
    ...
    async def test_send_message(self):
        message = {"channel": "/test/channel1", "data": {}, "clientId": None, "id": "1"}
        response = object()
        self.transport._send_payload_with_auth = mock.CoroutineMock(
            return_value=response
        )

        result = await self.transport._send_message(message, field="value")
    ...
    async def test_handshake(self):
        ...
        self.transport._send_message = mock.CoroutineMock(return_value=response)
        message = {
    ...
    def test_subscriptions(self):
        self.assertIs(self.transport.subscriptions, self.transport._subscriptions)
    ...
    async def test__connect(self):
        ...
        self.assertEqual(result, response)
        self.transport._send_payload_with_auth.assert_called_with([CONNECT_MESSAGE])
        self.assertFalse(self.transport._subscribe_on_connect)
    ...
    def test__state_initially_disconnected(self):
        self.assertIs(self.transport._state, TransportState.DISCONNECTED)

    def test_state(self):
        self.assertIs(self.transport.state, self.transport._state)
    ...
    async def test_start_connect_task(self, asyncio_mock):
        ...
        asyncio_mock.ensure_future.assert_called_with(coro, loop=None)
    ...
    async def test_connect_error_on_invalid_client_id(self):
        ...
        with self.assertRaisesRegex(
            TransportInvalidOperation,
            "Can't connect to the server without a " "client id. Do a handshake first.",
        ):
            await self.transport.connect()
    ...
    async def test_connect_done_with_result(self):
        cases = (
            ("successful", {"successful": True}, "retry"),
            ("unsuccessful without advice", {"successful": False}, "retry"),
            (
        ...
        for name, result, expected_advice in cases:
            with self.subTest(msg=name):
                ...
                self.transport._reconnect_advice = {"interval": 1}
                ...
                self.transport._follow_advice.assert_called_with(expected_advice, 1)
    ...
    async def test_connect_done_with_error(self):
        ...
        self.assertEqual(
            log.output, ["DEBUG:{}:{}".format(TransportBase.__module__, log_message)]
        )
    ...
    async def test_connect_dont_follow_advice_on_disconnecting(self):
        ...
        self.transport._reconnect_advice = {"interval": 1, "reconnect": "retry"}
        self.transport._reconnect_timeout = 2
    ...
    def test_follow_advice_handshake(self, defer):
        ...
        defer.assert_called_with(
            self.transport.handshake, delay=5, loop=None
        )
    ...
    def test_follow_advice_retry(self, defer):
        ...
        defer.assert_called_with(
            self.transport._connect, delay=5, loop=None
        )
    ...
    def test_follow_advice_none(self, defer):
        cases = (("none string", "none"), ("emtpy string", ""), ("None value", None))
            ...
            with self.subTest(msg=name):
                ...
                self.assertEqual(
                    ...
                    [
                        ...
                        " be scheduled.".format(TransportBase.__module__)
                    ],
                )
                ...
                self.assertEqual(
                    self.transport.state, TransportState.SERVER_DISCONNECTED
                )

    def test_client_id(self):
        self.assertIs(self.transport.client_id, self.transport._client_id)
    ...
    def test_endpoint(self):
        self.assertIs(self.transport.endpoint, self.transport._url)
    ...
    async def test_disconnect_transport_error(self):
        self.transport._state = TransportState.CONNECTED
        self.transport._stop_connect_task = mock.CoroutineMock()
        self.transport._send_message = mock.CoroutineMock(side_effect=TransportError())
    ...
    async def test_disconnect_if_not_connected(self):
        for state in (s for s in TransportState if s != TransportState.CONNECTED):
            self.transport._state = state
            self.transport._stop_connect_task = mock.CoroutineMock()
            self.transport._send_message = mock.CoroutineMock()
    ...
    async def test_subscribe(self):
        for state in [TransportState.CONNECTED, TransportState.CONNECTING]:
            self.transport._state = state
            self.transport._send_message = mock.CoroutineMock(return_value="result")

            result = await self.transport.subscribe("channel")

            self.assertEqual(result, self.transport._send_message.return_value)
            self.transport._send_message.assert_called_with(
    ...
    async def test_subscribe_error_if_not_connected(self):
        for state in TransportState:
            if state not in [TransportState.CONNECTED, TransportState.CONNECTING]:
                self.transport._state = state
                self.transport._send_message = mock.CoroutineMock(return_value="result")
    ...
    async def test_send_payload(self):
        ...
        response = object()
        self.transport._send_final_payload = mock.CoroutineMock(return_value=response)
        ...
        self.transport._send_final_payload.assert_called_with(payload, headers={})
    ...
    async def test_send_payload_with_auth(self):
        response = object()
        payload = object()
        self.transport._send_payload = mock.CoroutineMock(return_value=response)
        self.transport._auth = None
    ...
    def test_request_timeout_int(self):
        self.transport._reconnect_advice = {"timeout": 2000}
    ...
    def test_request_timeout_float(self):
        self.transport._reconnect_advice = {"timeout": 2000.0}
