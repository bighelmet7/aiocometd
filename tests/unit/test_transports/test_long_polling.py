...
class TestLongPollingTransport(TestCase):
    ...
    def test_connection_type(self):
        self.assertEqual(
            self.transport.connection_type, ConnectionType.LONG_POLLING
        )
    ...
    async def test_send_payload_final_payload(self):
        ...
        self.transport._consume_payload = mock.CoroutineMock(
            return_value=resp_data[0]
        )
        headers = dict(key="value")

        response = await self.transport._send_final_payload(
    ...
    async def test_send_payload_final_payload_client_error(self):
        ...
        with self.assertLogs(
            LongPollingTransport.__module__, level="DEBUG"
        ) as log:
        ...
        log_message = "WARNING:{}:Failed to send payload, {}".format(
            LongPollingTransport.__module__, post_exception
        )
    ...
    async def test_send_payload_final_payload_missing_response(self):
        ...
        payload = [object(), object()]
        self.transport.ssl = object()
        self.transport._consume_payload = mock.CoroutineMock(return_value=None)
        headers = dict(key="value")
        error_message = (
            "No response message received for the "
        ...
        log_message = "WARNING:{}:{}".format(
            LongPollingTransport.__module__, error_message
        )
        self.assertEqual(log.output, [log_message])
        self.transport._http_semaphore.__aenter__.assert_called()
        self.transport._http_semaphore.__aexit__.assert_called()
